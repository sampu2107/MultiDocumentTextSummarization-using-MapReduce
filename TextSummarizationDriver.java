...
public class TextSummarizationDriver extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		// call LDA
		boolean performLDA = performLDA(args);
		if (!performLDA) {
			return 1;
		}
	...
	// Performing LDA
	private boolean performLDA(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
		// TODO Auto-generated method stub

		Configuration conf = new Configuration();
		FileSystem fs = FileSystem.get(conf);
		Boolean jobStatus = true;
		Path result = new Path(args[5]);
		...
		for (FileStatus status : stati) {
			if (status.isDirectory() && jobStatus) {
				...
				FileOutputFormat.setOutputPath(job, new Path(args[19] + i));
